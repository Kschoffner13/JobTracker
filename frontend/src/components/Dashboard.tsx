...
function AnalyticsView({
  ...
  const { total, status_counts, weekly_applications, avg_days_to_interview } =
    analytics;
    ...
    <div>
      ...
      >
        ...
        <MetricCard
          ...
          value={
            avg_days_to_interview != null ? `${avg_days_to_interview}d` : "—"
          }
          color="#a855f7"
          t={t}
        />
          ...
          >
            {weekly_applications.map((w) => {
              const height = Math.max(
                Math.round((w.total / maxWeekly) * 120),
                4,
              );
              const weekLabel = new Date(w.week).toLocaleDateString("en-CA", {
                month: "short",
                day: "numeric",
              });
              return (
...
export function Dashboard() {
  ...
  const uniqueSources = Array.from(
    new Set(applications.map((a) => a.source).filter(Boolean)),
  ).sort() as string[];
  ...
  const filtered = applications.filter((a) => {
    ...
    if (filterTo && a.applied_at && a.applied_at > filterTo + "T23:59:59")
      return false;
    return true;
  });
  ...
  const counts = {
    ...
    interview: applications.filter((a) => a.current_status === "interview")
      .length,
    offer: applications.filter((a) => a.current_status === "offer").length,
    rejected: applications.filter((a) => a.current_status === "rejected")
      .length,
  };
        ...
        >
          <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
            {[
              ...
              { label: "Offer", value: counts.offer, color: "#10b981" },
